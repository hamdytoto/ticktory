  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        {/* Personal Info */}
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase">Personal Information</h3>
        ...
        {/* Account Info */}
        <div>
          <h3 className="text-xs font-semibold text-gray-500 uppercase">Account Status</h3>
          <div className="mt-4 space-y-4">
            <p className="text-sm text-gray-500">Account Type</p>
            <p className="text-gray-900 font-medium">{getUserRole(profile.type)}</p>
            <p className="text-sm text-gray-500">Status</p>
            <p className="text-gray-900 font-medium">{profile.status ? 'Active' : 'Inactive'}</p>
            <p className="text-sm text-gray-500">Last Login</p>
            <p className="text-gray-900 font-medium">{new Date(profile.last_login_time).toLocaleString()}</p>
          </div>
        </div>
      </div>
    </div>
