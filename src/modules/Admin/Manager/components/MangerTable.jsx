...
const ManagerTable = ({
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast.success(
        t("managerTable.deleteSuccess", "Manager deleted successfully"),
      );
      refetch();
    } catch (err) {
  ...
  const handleEditSubmit = async () => {
    await handleApiCallback(async () => {
      ...
      toast.success(
        t("managerTable.updateSuccess", "Manager updated successfully"),
      );
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <span className="text-gray-500 text-md">{row.service?.name}</span>
      ),
    },
    {
      ...
      render: (row) => (
        <span
          className={`font-medium ${row.automatic_assignment ? "text-green-900" : "text-red-900"}`}
        >
          {row.automatic_assignment
            ? t("managerTable.yes", "Yes")
            : t("managerTable.no", "No")}
        </span>
      ),
    },
    {
  ...
  const renderActions = (manager) => (
    <div
      className={`flex items-center ${isArabic ? "space-x-reverse space-x-3" : "space-x-3"}`}
    >
      <button
  ...
  return (
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg">
      ...
      <ConfirmDialog
        show={showConfirm}
        message={t(
          "managerTable.confirmDelete",
          "Do you really want to delete this manager? It cannot be undone.",
        )}
