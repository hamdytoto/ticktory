  ...
  const columns = [
    ...
    },
    {
      ...
      render: (ticket) => {
        ...
        return new Date(ticket.assigned_at).toLocaleString(
      ...
      render: (ticket) => {
        ...
        return (
          <span
            ...
            title={t(
              ticket.is_overdue
                ? "tickets.tooltip.overdue"
                : "tickets.tooltip.onTime",
            )}
          >
            {ticket.is_overdue ? (
              <>
      ...
      render: (ticket) =>
        ticket.status === 3 ? (
          <span
            ...
            title={t("tickets.tooltip.closed")}
          >
          ...
          <div className="flex items-center space-x-3">
            <button
              ...
              title={t("tickets.tooltip.assignTechnician")}
            >
              {<FaTools />}
            </button>
            <button
              onClick={() => handleResolveClick(ticket)}
              className={`${ticket.status === 2 ? "text-green-600 cursor-default" : "text-yellow-500 hover:text-yellow-600 cursor-pointer"}`}
              title={
                ticket.status === 2
    ...
    <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg">
      ...
      <div className="mb-4 flex justify-between items-center">
        <div className="text-sm text-gray-600">
          {t("table.pagination.showing", "Showing")}{" "}
          {ticketsData.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}{" "}
          {t("table.pagination.to", "to")}{" "}
          {Math.min(currentPage * itemsPerPage, totalRecords)}{" "}
          {t("table.pagination.of", "of")} {totalRecords}{" "}
          {t("table.pagination.tickets", "tickets")}
        </div>
        <div className="text-sm text-gray-500">
          {t("table.pagination.page", "Page")} {currentPage}{" "}
          {t("table.pagination.of", "of")} {totalPages}
        </div>
      </div>
